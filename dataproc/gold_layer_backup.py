...
# release_events may not exist on first run — create empty DataFrame if missing
_release_path = f"{SILVER}/release_events"
if DeltaTable.isDeltaTable(spark, _release_path):
    releases = spark.read.format("delta").load(_release_path)
else:
    from pyspark.sql.types import StructType, StructField, StringType, LongType, BooleanType, TimestampType, ShortType
    _release_schema = StructType([
        StructField("event_id",            StringType()),
        StructField("repo_id",             LongType()),
        StructField("repo_name",           StringType()),
        StructField("actor_id",            LongType()),
        StructField("actor_login",         StringType()),
        StructField("created_at",          TimestampType()),
        StructField("year",                ShortType()),
        StructField("action",              StringType()),
        StructField("release_id",          LongType()),
        StructField("tag_name",            StringType()),
        StructField("release_name",        StringType()),
        StructField("is_prerelease",       BooleanType()),
        StructField("is_draft",            BooleanType()),
        StructField("release_created_at",  TimestampType()),
        StructField("published_at",        TimestampType()),
        StructField("source_file",         StringType()),
    ])
    releases = spark.createDataFrame([], _release_schema)
    print("[gold] release_events table not found — using empty DataFrame")
...
lang_dim    = spark.read.format("delta").load(f"{GOLD}/dim_language")
lang_map    = lang_dim.select(F.lower("language_name").alias("lang_lower"), "language_key")\
                      .dropDuplicates(["lang_lower"])
date_map    = spark.read.format("delta").load(f"{GOLD}/dim_date")\
                   .select("date_key")\
                   .dropDuplicates(["date_key"])

# Dynamic lookup for "Unknown" language key instead of hardcoded value
_unknown_rows = lang_dim.filter(F.col("language_name") == "Unknown").select("language_key").collect()
_unknown_lang_key = _unknown_rows[0]["language_key"] if _unknown_rows else -1
...
def attach_keys(df, ts_col="created_at"):
    ...
    return (
        ...
        .withColumn("language_key",
            F.coalesce(F.col("language_key"), F.lit(_unknown_lang_key)))
    )
...
issues_fact = (
    ...
    .select("event_id", "date_key", "repo_key", "contributor_key",
            "language_key", "year", "action", "issue_id", "label_list",
...
# fact_release_events
# ================================================================
watch_keyed   = attach_keys(watches).dropDuplicates(["event_id"])
fork_keyed    = attach_keys(forks).dropDuplicates(["event_id"])
release_keyed = attach_keys(releases).dropDuplicates(["event_id"])

release_fact = (
    release_keyed
    .select("event_id", "date_key", "repo_key", "contributor_key",
            "language_key", "year", "action", "release_id",
            "tag_name", "release_name", "is_prerelease", "is_draft",
            "release_created_at", "published_at")
    .dropDuplicates(["event_id"])
)
if releases.count() > 0:
    write_gold(release_fact, "fact_release_events",
               merge_key="event_id", partition_col="year")
else:
    print("  [gold] SKIP → fact_release_events (no release data yet)")

# ================================================================
# fact_repo_activity_daily
# ================================================================

# Build a spine of all (date_key, repo_key) from ALL event types
# so repos with only stars/PRs/issues/forks/releases (no pushes) are included
# Use first() for language_key/year to avoid duplicates on the merge key
_spine_cols = ["date_key", "repo_key", "language_key", "year"]
activity_spine = (
    push_fact.select(*_spine_cols)
    .union(pr_fact.select(*_spine_cols))
    .union(issues_fact.select(*_spine_cols))
    .union(watch_keyed.select(*_spine_cols))
    .union(fork_keyed.select(*_spine_cols))
    .union(release_keyed.select(*_spine_cols))
    .groupBy("date_key", "repo_key")
    .agg(F.first("language_key").alias("language_key"),
...
release_agg = (
    release_keyed.groupBy("date_key", "repo_key")
                 .agg(F.count("event_id").alias("total_releases"))
)
...
_DAILY_METRICS = [
    ...
    "total_stars", "total_forks", "total_releases",
]
daily = (
    activity_spine
    .join(push_agg,    ["date_key", "repo_key"], "left")
    ...
    .join(release_agg, ["date_key", "repo_key"], "left")
...
# Aggregate each event type by language + year
_push_by_lang = (
    push_fact.groupBy("language_key", "year")
    .agg(F.count("event_id").alias("total_push_events"),
         F.sum("commit_count").alias("total_commits"),
         F.countDistinct("repo_key").alias("push_unique_repos"),
         F.countDistinct("contributor_key").alias("push_unique_contributors"))
)
_pr_by_lang = (
    pr_fact.groupBy("language_key", "year")
    .agg(F.count("event_id").alias("total_pr_events"),
         F.sum(F.col("is_merged").cast("int")).alias("total_prs_merged"),
         F.avg("time_to_merge_hours").alias("avg_pr_merge_hours"))
)
_iss_by_lang = (
    issues_fact.groupBy("language_key", "year")
    .agg(F.count("event_id").alias("total_issue_events"),
         F.sum((F.col("action") == "opened").cast("int")).alias("total_issues_opened"),
         F.sum((F.col("action") == "closed").cast("int")).alias("total_issues_closed"))
)
_star_by_lang = (
    watch_keyed.groupBy("language_key", "year")
    .agg(F.count("event_id").alias("total_stars"))
)
_fork_by_lang = (
    fork_keyed.groupBy("language_key", "year")
    .agg(F.count("event_id").alias("total_forks"))
)

lang_yearly = (
    _push_by_lang
    .join(_pr_by_lang,   ["language_key", "year"], "full")
    .join(_iss_by_lang,  ["language_key", "year"], "full")
    .join(_star_by_lang, ["language_key", "year"], "full")
    .join(_fork_by_lang, ["language_key", "year"], "full")
    .fillna(0, subset=["total_push_events", "total_commits",
                        "push_unique_repos", "push_unique_contributors",
                        "total_pr_events", "total_prs_merged",
                        "total_issue_events", "total_issues_opened",
                        "total_issues_closed", "total_stars", "total_forks"])
    .withColumn("unique_repos", F.col("push_unique_repos"))
    .withColumn("unique_contributors", F.col("push_unique_contributors"))
...
gold_tables = [
    ...
    "fact_release_events", "fact_repo_activity_daily", "fact_language_yearly",
]
...
for tbl in bq_tables:
    _gold_path = f"{GOLD}/{tbl}"
    if not DeltaTable.isDeltaTable(spark, _gold_path):
        print(f"  [gold→BQ] SKIP {tbl} (gold table not found yet)")
        continue
    df = (spark.read.format("delta").load(_gold_path)
